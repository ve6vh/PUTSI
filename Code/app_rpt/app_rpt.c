...
static int uchameleon_connect(struct daq_entry_tag *t)
{
	...
	char rxbuf[20];
        ...
        if((count = serial_io(t->fd, idbuf, rxbuf, strlen(idbuf), 14, DAQ_RX_TIMEOUT, 0x0a)) < 1){
              	ast_log(LOG_WARNING, "serial_io on %s failed: %d\n", t->name, errno);
		close(t->fd);
		t->fd = -1;
                return -1;
        }
	...
	if((count != 13)||(strncmp(expect, rxbuf+4, sizeof(&expect)))){
		ast_log(LOG_WARNING, "%s is not a uchameleon device %d(%s)\n", t->name, count, rxbuf);
		close(t->fd);
		t->fd = -1;
		return -1;
	}
	...
	if((argc == 6)&&(s[0] != '-')){
		for(i = 0; i < nrpts; i++){
			if(!strcmp(argv[3], rpt_vars[i].name)){
				...
				ast_log(LOG_NOTICE, "Alarm response: sending %s to node %s\n", s, rpt_vars[i].name);
...
static void *uchameleon_monitor_thread(void *this)
{
	...
	for(;;){
		...
		if(res == -1){
			ast_log(LOG_ERROR,"serial_rx failed: %d %s\n", res, rxbuff);
			close(t->fd);
			ast_mutex_lock(&t->lock);
			t->fd = -1;
			t->active = 0;
		...
		if(res){
			if(debug >= 5) 
				ast_log(LOG_NOTICE, "Received: %d:%s\n",res, rxbuff);
			valid = 0;
			/* Parse return string */
			...
			if(i == 3){
				if(!strcmp(rxargs[0],"pin")){
					...
					if(debug >= 4) 
						ast_log(LOG_NOTICE, "Pin Msg: %d:%d\n",pin, sample);
				}
				if(!strcmp(rxargs[0],"adc")){
					...
					sample = atoi(rxargs[2]);
					if(debug >= 4) 
						ast_log(LOG_NOTICE, "ADC Msg: %d:%d\n",pin, sample);
				}
			}
			if(valid){
...
struct ast_channel *mychannel,*genchannel,*c;
	...
	/*
	 * try all the fd's to see if something works...
	 */
	int tzr=0;
	if (myrpt->p.tonezone && (tone_zone_set_zone(genchannel->fds[0],myrpt->p.tonezone) == -1))
	{
		ast_log(LOG_WARNING, "Unable to set tone zone %s:%d\n",myrpt->p.tonezone, errno);
		ast_hangup(mychannel);
		ast_hangup(genchannel);
		myrpt->callmode = 0;
		pthread_exit(NULL);
	}
	ast_log(LOG_NOTICE, "Set tone zone %s successful\n",myrpt->p.tonezone);
	...
	{
		ast_log(LOG_WARNING, "Cannot start dialtone: %d:%d:%d(%d) [%d]\n",genchannel->fds[1],myrpt->patchquiet,myrpt->patchexten[0], tzr, errno);
		ast_hangup(mychannel);
		ast_hangup(genchannel);
		myrpt->callmode = 0;
		pthread_exit(NULL);
...
static int function_autopatchup(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	...
	if(param){
		...
		ast_log(LOG_NOTICE, "AP up, param string %s\n", param);
		...
		ast_log(LOG_NOTICE, "AP up, params: context=%s,dial=%d,farend=%d,noct=%d,quiet=%d,vox=%d,exten=%s\n", 
				myrpt->patchcontext, myrpt->patchdialtime, myrpt->patchfarenddisconnect, myrpt->patchnoct,
				myrpt->patchquiet, myrpt->patchvoxalways, myrpt->patchexten); 
		ast_free(lparam);
	}
...
static int function_autopatchdn(struct rpt *myrpt, char *param, char *digitbuf, int command_source, struct rpt_link *mylink)
{
	ast_log(LOG_NOTICE,"Autopatch DN entered");
...
char	cmd[MAXDTMF+1] = "",c,tone[10];
	...
	if (!myrpt->p.simple)
	{
		...
		{
			time(&myrpt->dtmf_time);
			ast_log(LOG_NOTICE, "cancel_pfxfone: %d %c %x %d!\n",myrpt->inpadtest, c, myrpt->p.endchar, myrpt->dtmfidx);
